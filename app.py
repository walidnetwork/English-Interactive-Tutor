...
if "GEMINI_API_KEY" in st.secrets:
    ...
    st.error("⚠️ مفتاح GEMINI_API_KEY غير مضبوط في Secrets")
    st.stop()
...
def get_pdf_text(path, p_num):
    try:
        ...
        if 1 <= p_num <= len(doc):
            ...
            return text if text else "⚠️ الصفحة فارغة أو عبارة عن صور."
...
if submit_btn:
    with st.spinner("⏳ جاري التحليل..."):
        ...
        if not extracted_text.startswith("❌") and not extracted_text.startswith("⚠️"):
            st.success(f"✅ تم قراءة الصفحة {page_num}")
            
            prompt = f"أنت معلم إنجليزي خبير. اشرح الأسئلة التالية بأسلوب ممتع بالعربي:\n{extracted_text}"
